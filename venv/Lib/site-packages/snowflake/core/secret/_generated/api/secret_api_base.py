...
from .secret_api import SecretApi
...
class SecretCollectionBase(SchemaObjectCollectionParent["SecretResource"]):
    ...
    @api_telemetry
    def create(self, secret: Secret, mode: Optional[Union[CreateMode, str]] = None) -> "SecretResource":  # noqa: F821
        """Create a secret.
        ...
        if self._identifier_requires_args:
            return self._ref_class(get_function_name_with_args(secret), self)
        return self._ref_class(secret.name, self)

    @api_telemetry
    def create_async(
        self, secret: Secret, mode: Optional[Union[CreateMode, str]] = None
    ) -> PollingOperation["SecretResource"]:  # noqa: F821
        ...
        future = self._api.create_secret(
            self.database.name,
            self.schema.name,
            secret=secret,
            create_mode=mode,
        ...
        if self._identifier_requires_args:
            return PollingOperation(future, lambda _: self._ref_class(get_function_name_with_args(secret), self))
        return PollingOperation(
            future, lambda _: self._ref_class(secret if isinstance(secret, str) else secret.name, self)
        )
    ...
    def iter(
        ...
        *,
        ...
        resources = self._api.list_secrets(
            self.database.name,
            self.schema.name,
            like=like,
    ...
    def iter_async(
        ...
        *,
        ...
        from_name: Optional[str] = None,
        show_limit: Optional[int] = None,
    ) -> PollingOperation[Iterator[Secret]]:
...
class SecretResourceBase(SchemaObjectReferenceMixin["SecretCollection"]):
    _collection_class: SecretCollectionBase
    _identifier_requires_args: bool = False
    _plural_name: str

    def __init__(self, name: str, collection_class: SecretCollectionBase) -> None:
    ...
    def fetch_async(
        self,
    ) -> PollingOperation[Secret]:
        """An asynchronous version of :func:`fetch`.

        Refer to :class:`~snowflake.core.PollingOperation` for more information on asynchronous execution and
        the return type.
        """
        future = self.collection._api.fetch_secret(
            self.database.name,
            self.schema.name,
            self._identifier,
            async_req=True,
    ...
    def drop(
        ...
        """
        self.collection._api.delete_secret(
            self.database.name,
            self.schema.name,
            self._identifier,
            if_exists=if_exists,
    ...
    def drop_async(
        ...
        the return type.
        """
        future = self.collection._api.delete_secret(
            self.database.name,
            self.schema.name,
            self._identifier,
            if_exists=if_exists,
