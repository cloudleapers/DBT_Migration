...
class StreamCollectionBase(SchemaObjectCollectionParent["StreamResource"]):
    ...
    @api_telemetry
    def create(
        self, stream: Stream, mode: Optional[Union[CreateMode, str]] = None, copy_grants: Optional[bool] = None
    ) -> "StreamResource":  # noqa: F821
        """Create a stream.
        ...
        if self._identifier_requires_args:
            return self._ref_class(get_function_name_with_args(stream), self)
        return self._ref_class(stream.name, self)

    @api_telemetry
    def create_async(
        self, stream: Stream, mode: Optional[Union[CreateMode, str]] = None, copy_grants: Optional[bool] = None
    ) -> PollingOperation["StreamResource"]:  # noqa: F821
        ...
        future = self._api.create_stream(
            self.database.name,
            self.schema.name,
            stream=stream,
            create_mode=mode,
        ...
        if self._identifier_requires_args:
            return PollingOperation(future, lambda _: self._ref_class(get_function_name_with_args(stream), self))
        return PollingOperation(
            future, lambda _: self._ref_class(stream if isinstance(stream, str) else stream.name, self)
        )
    ...
    def iter(
        ...
        *,
        ...
        resources = self._api.list_streams(
            self.database.name,
            self.schema.name,
            like=like,
...
class StreamResourceBase(SchemaObjectReferenceMixin["StreamCollection"]):
    _collection_class: StreamCollectionBase
    _identifier_requires_args: bool = False
    _plural_name: str

    def __init__(self, name: str, collection_class: StreamCollectionBase) -> None:
    ...
    def clone(
        ...
        copy_grants: bool
             Parameter to enable copy grants when creating the object.
        """
        self.collection._api.clone_stream(
            self.database.name,
            self.schema.name,
            self._identifier,
    ...
    def clone_async(
        ...
        the return type.
        """
        future = self.collection._api.clone_stream(
            self.database.name,
            self.schema.name,
            self._identifier,
            target_database=target_database,
    ...
    def fetch(
        ...
        __________
        """
        result = self.collection._api.fetch_stream(
            self.database.name,
            self.schema.name,
            self._identifier,
            async_req=False,
    ...
    def get_tags(
        ...
        """Get the tag assignments for a stream.

        Returns all tags assigned to a stream. This operation requires an active warehouse.
