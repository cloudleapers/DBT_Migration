...
from .image_repository_api import ImageRepositoryApi
...
class ImageRepositoryCollectionBase(SchemaObjectCollectionParent["ImageRepositoryResource"]):
    ...
    @api_telemetry
    def create(
        self, image_repository: ImageRepository, mode: Optional[Union[CreateMode, str]] = None
    ) -> "ImageRepositoryResource":  # noqa: F821
        ...
        if self._identifier_requires_args:
            return self._ref_class(get_function_name_with_args(image_repository), self)
        return self._ref_class(image_repository.name, self)

    @api_telemetry
    def create_async(
        self, image_repository: ImageRepository, mode: Optional[Union[CreateMode, str]] = None
    ) -> PollingOperation["ImageRepositoryResource"]:  # noqa: F821
        ...
        if self._identifier_requires_args:
            return PollingOperation(
                future, lambda _: self._ref_class(get_function_name_with_args(image_repository), self)
            )
        return PollingOperation(
            future,
            lambda _: self._ref_class(
                image_repository if isinstance(image_repository, str) else image_repository.name, self
            ),
        )
    ...
    def iter(self, *, like: Optional[str] = None) -> Iterator[ImageRepository]:
        ...
        """
        resources = self._api.list_image_repositories(
            self.database.name,
            self.schema.name,
            like=like,
            async_req=False,
...
class ImageRepositoryResourceBase(SchemaObjectReferenceMixin["ImageRepositoryCollection"]):
    _collection_class: ImageRepositoryCollectionBase
    _identifier_requires_args: bool = False
    _plural_name: str

    def __init__(self, name: str, collection_class: ImageRepositoryCollectionBase) -> None:
    ...
    def fetch(
        ...
        __________
        """
        result = self.collection._api.fetch_image_repository(
            self.database.name,
            self.schema.name,
            self._identifier,
            async_req=False,
    ...
    def fetch_async(
        self,
    ) -> PollingOperation[ImageRepository]:
    ...
    def get_tags(
        ...
        Returns all tags assigned to an image repository. This operation requires an active warehouse.
    ...
    def list_images_in_repository(
        ...
        __________
        """
        result = self.collection._api.list_images_in_repository(
            self.database.name,
            self.schema.name,
            self._identifier,
            async_req=False,
    ...
    @api_telemetry
    def list_images_in_repository_async(
        self,
    ) -> PollingOperation[Iterable[Image]]:
    ...
    def set_tags(
        ...
        """Set tags on an image repository.
    ...
    def unset_tags(
        ...
        """Unset tags from an image repository.
